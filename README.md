...
In basic terms, this guide will help you launch a cloud computer (instance) that will live in a data center owned by Amazon. First, you will need to set some basic specifications for your instance.

Once logged into your dashboard, click "Instances," located on the left hand sidebar menu.

This will take you to a dashboard that displays instances you currently have running. You haven't launched your instance yet, so you won't see anything here.

Now, in the top right corner, click "Launch Instances." 

This opens "Launch an Instance." Below the heading you will see a box called "Name and tags." In the text box, type a descriptive name for your instance, e.g., "My first instance" or "My app server."
...
Now, click "Create key pair." A file (<myappname>.pem) will be downloaded to your computer. Note the location—you will need to locate this file on your computer in the next section.
  
### Network settings  

The next section is called "Network settings." Leave the default selections or now and move on to "Configure storage."

At the time of this writing, free-tier-eligible customers can get up to 30 GB of EBS General Purpose (SSD) or Magnetic storage, so you can change 8GB to 30GB.

To see what other options are available, click the "Advanced details," but there is no need to change any of these settings right now.

### Lift off!

In this section, you will connect our pair and launch our Amazon EC2 instance. 
...
Before you access your instance, you will need to secure your connection. In the previous section, you downloaded a .pem file. By default, it will have been downloaded to ```/Downloads```. To secure the file from potential leaks, connect to our instance via SSH (secure shell). Going forward, this guide will assume that your .pem file is located in ```~/Downloads```, but you can replace that with the file path where you've stored it.
...
Open your computer's terminal. If you are a Mac user, there is a chance your .```.ssh``` directory has not been created. First, check to see if your ```.ssh``` directory exists. In your terminal, type: 
...
ls -a
```
This will list all files and directories, including hidden directories. Now, try and locate the ```.ssh``` directory. If it has already been created, you will see ```.ssh``` in your list of directories. If you don't see it listed, type: ```mkdir .ssh```, then press enter to create a ```.ssh``` directory inside your home directory.

Now that you've ensured we have an ```.ssh``` directory, you'll now use the terminal to relocate your .pem file.
...
Mac users, type one more command to remove the ```@```:
```
xattr -c <myapp>.pem
```
...
This means you have successfully changed the permissions and placed the .pem file in the ```.ssh``` directory. Now, you can connect to our EC2 instance via the terminal. 

### Using SSH to connect to our EC2 instance

Now that you can use SSH to connect to our EC2 instance, you will also need the public IPv4 address of your new instance. 

This can be retrieved from the AWS console by navigating to the EC2 dashboard and click "Instances."
...
If you see this, type ```yes``` and click enter.

Your shell with now show ubuntu@ip-OUR-ELASTIC-IP. This means you are now logged in as the default user, ubuntu, on our computer in the cloud. To test if you are successfully logged in, type ```echo Hello Cloud!``` and press ```Enter```.

Your computer just said, "Hello!"
  
Congratulations! You now have secure access to your Amazon EC2 Instance.

NOTE: If you press ```Ctrl-D``` on during your SSH session, it will disconnect the SSH session and our shell will be back on our local machine.  To reconnect, re-enter the SSH command from above. Or you can press the up arrow. (If you just disconnected the ssh command will be the last command you entered locally.)
